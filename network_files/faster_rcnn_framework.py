...
class FasterRCNNBase(nn.Module):
    ...
    def __init__(self, backbone, rpn, roi_heads, transform, loss_fn, focal, cbam, double_fusion, val, weight):
    ...
    def eager_outputs(self, losses, detections):
        # type: (Dict[str, Tensor], List[Dict[str, Tensor]]) -> Union[Dict[str, Tensor], Tuple[List[Dict[str, Tensor]], Dict[str, Tensor]]]
        if self.training:
            return losses
    ...
    def forward(self, images, targets=None):
        ...
        proposals, proposal_losses = self.rpn(images, features, self.loss_fn, self.focal, self.val, self.weight, targets)

        # 将rpn生成的数据以及标注target信息传入fast rcnn后半部分
        detections, detector_losses = self.roi_heads(features, proposals, images.image_sizes, self.loss_fn, self.focal, self.val, self.weight, targets)
...
class FasterRCNN(FasterRCNNBase):
    ...
    def __init__(self, backbone, num_classes=None,
        ...
        super(FasterRCNN, self).__init__(backbone, rpn, roi_heads, transform, loss_fn, focal, cbam, double_fusion, val, weight)
